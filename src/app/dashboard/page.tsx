...
export default function Dashboard() {
  ...
  const checkGymSubmissionToday = async () => {
    try {
  ...
  const handleLeetCodeSubmit = async (data: { problemName: string; solvedWithHelp: boolean; learnings: string }) => {
    try {
  ...
  const handleGymSubmit = async (data: { workout: string }) => {
    try {
      ...
      alert(error.message || "Error submitting gym workout")
    }
  }
      ...
      <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <p className="mt-2 text-gt-gray">
            Track your daily progress and stay accountable with the Yellow Jacket community.
          </p>
        </div>
